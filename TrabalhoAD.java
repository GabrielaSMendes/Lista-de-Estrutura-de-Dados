...
public class TrabalhoAD {
    ...
    static class BST<T extends Comparable<T>> {
        ...
        /** Remoção O(h) */
        public void remove(T k) { root = rem(root, k); }
        private Node rem(Node n, T k) {
            ...
            if      (c < 0) n.left  = rem(n.left,  k);
                ...
                Node m = min(n.right); n.key = m.key;
    ...
    static class AVL<T extends Comparable<T>> {
        ...
        private void upH(Node n) { n.height = 1 + Math.max(h(n.left), h(n.right)); }
        ...
        private Node bal(Node n) {
            upH(n);
            if (bf(n) >  1) { if (bf(n.left)  < 0) n.left  = rotL(n.left);  return rotR(n); }
            if (bf(n) < -1) { if (bf(n.right) > 0) n.right = rotR(n.right); return rotL(n); }
            return n;
        }
    ...
    static class RedBlack<T extends Comparable<T>> {
        ...
        private class Node {
            T key; Node left, right; boolean color;
            Node(T k) { key = k; color = R; }
        }

        private boolean isR(Node n) { return n != null && n.color == R; }

        private Node rotL(Node x) {
            Node y = x.right; x.right = y.left; y.left = x;
            y.color = x.color; x.color = R; return y;
        }
        private Node rotR(Node y) {
            Node x = y.left; y.left = x.right; x.right = y;
            x.color = y.color; y.color = R; return x;
        }
        private void flip(Node n) {
            n.color = R; n.left.color = B; n.right.color = B;
        }

        /** Inserção O(log n) */
        public void insert(T k) { root = ins(root, k); root.color = B; }
        private Node ins(Node n, T k) {
            if (n == null) return new Node(k);
            int c = k.compareTo(n.key);
            if      (c < 0) n.left  = ins(n.left,  k);
        ...
        private Node mvL(Node n) {
            flip(n);
            if (isR(n.right.left)) { n.right = rotR(n.right); n = rotL(n); flip(n); }
            return n;
        }
        private Node mvR(Node n) {
            flip(n);
            if (isR(n.left.left)) { n = rotR(n); flip(n); }
            return n;
        }
    ...
    static class TSP {
        ...
        TSP(double[][] d) { this.d = d; this.n = d.length; }
        ...
        int[] nearestNeighbor(int start) {
            ...
            int[] tour = new int[n];
            tour[0] = start; vis[start] = true;
            for (int i = 1; i < n; i++) {
                int cur = tour[i-1], best = -1; double bd = Double.MAX_VALUE;
                for (int j = 0; j < n; j++)
                    if (!vis[j] && d[cur][j] < bd) { bd = d[cur][j]; best = j; }
                tour[i] = best; vis[best] = true;
            }
            return tour;
        }
            ...
            while (improved) {
                ...
                for (int i = 0; i < n-1; i++) {
                    for (int j = i+2; j < n; j++) {
                        ...
                        if (delta < -1e-10) { rev(t, i+1, j); improved = true; }
                    }
                }
            }
            return t;
        ...
        double len(int[] t) {
            double s = 0; for (int i = 0; i < n; i++) s += d[t[i]][t[(i+1)%n]]; return s;
        }

        /** NN + 2-opt combinados */
        double solve() { return len(twoOpt(nearestNeighbor(0))); }
    }
    ...
    static double std(double[] v)    { double m=mean(v),s=0; for (double x:v) s+=(x-m)*(x-m); return Math.sqrt(s/(v.length-1)); }
    ...
    static void runTrees() {
        ...
        for (int N : sizes) {
            ...
            for (int r = 0; r < RUNS; r++) {
                ...
                // Inserção
                long t;
                t = System.nanoTime(); for (int v:data) bst.insert(v); ins[0][r]  = (System.nanoTime()-t)/1e6;
                t = System.nanoTime(); for (int v:data) avl.insert(v); ins[1][r]  = (System.nanoTime()-t)/1e6;
                t = System.nanoTime(); for (int v:data) rb.insert(v);  ins[2][r]  = (System.nanoTime()-t)/1e6;

                // Busca (1000 ops aleatórias)
                t = System.nanoTime(); for (int i=0;i<1000;i++) bst.search(data[rng.nextInt(N)]); srch[0][r]=(System.nanoTime()-t)/1e6;
                t = System.nanoTime(); for (int i=0;i<1000;i++) avl.search(data[rng.nextInt(N)]); srch[1][r]=(System.nanoTime()-t)/1e6;
                t = System.nanoTime(); for (int i=0;i<1000;i++) rb.search(data[rng.nextInt(N)]);  srch[2][r]=(System.nanoTime()-t)/1e6;

                // Remoção (500 ops aleatórias)
                t = System.nanoTime(); for (int i=0;i<500;i++) bst.remove(data[rng.nextInt(N)]); rem[0][r]=(System.nanoTime()-t)/1e6;
                t = System.nanoTime(); for (int i=0;i<500;i++) avl.remove(data[rng.nextInt(N)]); rem[1][r]=(System.nanoTime()-t)/1e6;
                t = System.nanoTime(); for (int i=0;i<500;i++) rb.remove(data[rng.nextInt(N)]);  rem[2][r]=(System.nanoTime()-t)/1e6;
